...
function renderPokemon(pokemon) {
  ...
  divText.setAttribute('class', 'pokemon-card-text');
  imgEl.setAttribute('src', pokemon.imageUrl);

  pEl.textContent = pokemon.description;
  h3El.textContent = pokemon.number;
...
for (var i = 0; i < pokedex.length; i++) {
  var pokeLog = renderPokemon(pokedex[i]);
  divRow.appendChild(pokeLog);
}
